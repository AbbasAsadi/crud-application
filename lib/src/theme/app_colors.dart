import 'dart:ui';

class AppColors {
  static const primaryLight = Color(0xff94cbfd);
  static const primaryMedium = Color(0xff007FF4);
  static const primaryDark = Color(0xff0157a4);

  static const secondaryLight = Color(0xFFF4007F);
  static const secondaryMedium = Color(0xFFEF00F4);
  static const secondaryDark = Color(0xFF7500F4);

  static const white = Color(0xFFFFFFFF);
  static const gray25 = Color(0xFF797979);
  static const gray50 = Color(0xFFA0A0A0);
  static const gray100 = Color(0xFFDCEDFC);
  static const gray200 = Color(0xFFBFDFFC);
  static const gray300 = Color(0xFF9FCFFB);
  static const gray400 = Color(0xFF80BFFA);
  static const gray500 = Color(0xFF60AFF8);
  static const gray600 = Color(0xFF409FF7);
  static const gray800 = Color(0xFF5D5D5D);
  static const gray850 = Color(0xFF323232);
  static const gray900 = Color(0xFF1C1C1C);
  static const black = Color(0xFF000000);


  static const shadow100 = Color(0xFF00407A);
  static const shadow300 = Color(0xFF00305B);
  static const shadow500 = Color(0xFF00203D);
  static const shadow700 = Color(0xFF00101E);




  static const redError = Color(0xFFB00020);

}
